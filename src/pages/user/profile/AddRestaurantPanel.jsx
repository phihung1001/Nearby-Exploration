...
import { registerRestaurantAPI } from "../../../services/restaurantService";
...
const AddRestaurantPanel = ({ onCancel }) => {
  ...
  const onFinish = async (values) => {
    try {
      ...
      await registerRestaurantAPI(values, token);

      notification.success({
        message: "Thêm mới thành công",
        description: "Nhà hàng đã được thêm vào hệ thống.",
      });

      form.resetFields();
      onCancel(); // Trở lại danh sách hoặc đóng form
    } catch (error) {
      notification.error({
  ...
  return (
    <Card title="Thêm Nhà Hàng Mới" style={{ maxWidth: 600, margin: "0 auto" }}>
      <Form form={form} layout="vertical" onFinish={onFinish}>
        ...
        <Form.Item label="Link ảnh" name="photoUrl">
          <Input placeholder="Nhập link ảnh" />
        </Form.Item>

        <Form.Item>
          <Button type="primary" htmlType="submit">Thêm Nhà Hàng</Button>
